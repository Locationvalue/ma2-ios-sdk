...
@class NSURL;
@class NSNumber;
SWIFT_ENUM_FWD_DECL(NSInteger, NautilusLotteryType)
/// 抽選データ
SWIFT_CLASS("_TtC18NautilusLotterySDK19NautilusLotteryInfo")
@interface NautilusLotteryInfo : NSObject
/// 抽選のID
@property (nonatomic, readonly) NSInteger lotteryID;
/// 抽選のタイトル
@property (nonatomic, readonly, copy) NSString * _Nullable lotteryTitle;
/// エイリアス
@property (nonatomic, readonly, copy) NSString * _Nullable alias;
/// 画像のURL
@property (nonatomic, readonly, copy) NSURL * _Nullable imageURL;
/// 画像のアスペクト比（NSNumber版）
@property (nonatomic, readonly, strong) NSNumber * _Nullable imageAspectRatioNumber;
/// 抽選の掲載開始日時
...
/// 抽選の種別
@property (nonatomic, readonly) enum NautilusLotteryType lotteryType;
/// 抽選を繰り返すか
...
@class NautilusLotteryPrizeInfo;
/// 抽選の間隔確認の結果
...
/// 間隔の確認結果
/// true: 抽選申し込み可能, false: 抽選済み
@property (nonatomic, readonly) BOOL checkResult;
/// 抽選結果のランクをNSNumber型で取得する。rankがnilの場合はnilを返す
@property (nonatomic, readonly, strong) NSNumber * _Nullable rankNumber;
/// 抽選結果の景品
@property (nonatomic, readonly, strong) NautilusLotteryPrizeInfo * _Nullable prize;
...
/// 交換IDをNSNumber型で取得する。exchangeIDがnilの場合はnilを返す
...
@class NautilusLotteryPrizePointInfo;
/// 抽選結果となる景品のデータ
SWIFT_CLASS("_TtC18NautilusLotterySDK24NautilusLotteryPrizeInfo")
@interface NautilusLotteryPrizeInfo : NSObject
/// 抽選の景品IDをNSNumber型で取得する。prizeIDがnilの場合はnilを返す
@property (nonatomic, readonly, strong) NSNumber * _Nullable prizeIDNumber;
/// 抽選の景品種別をNSNumber型で取得する。prizeTypeがnilの場合はnilを返す
@property (nonatomic, readonly, strong) NSNumber * _Nullable prizeTypeNumber;
/// 抽選の景品となるクーポンの情報
@property (nonatomic, readonly, strong) NautilusLotteryPrizeCouponInfo * _Nullable coupon;
/// 抽選の景品となるポイントの情報
@property (nonatomic, readonly, strong) NautilusLotteryPrizePointInfo * _Nullable point;
/// 抽選結果は必ずサーバから取得する
/// SDK外から初期化をさせない
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
...
/// 付与ポイントをNSNumber型で取得する。addPointがnilの場合はnilを返す
@property (nonatomic, readonly, strong) NSNumber * _Nullable addPointNumber;
/// ポイントデータは必ずサーバから取得する
/// SDK外から初期化をさせない
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
