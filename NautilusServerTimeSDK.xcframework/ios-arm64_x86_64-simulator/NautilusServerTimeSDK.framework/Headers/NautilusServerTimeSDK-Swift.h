...
#ifndef NAUTILUSSERVERTIMESDK_SWIFT_H
#define NAUTILUSSERVERTIMESDK_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="NautilusServerTimeSDK",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// サーバー時刻の管理状態
typedef SWIFT_ENUM(NSInteger, NautilusServerTimeStatus, open) {
/// サーバー時刻は無効な状態
/// サーバー時刻を基準とした日時の取得はできない
  NautilusServerTimeStatusInvalid = 0,
/// サーバー時刻の更新・同期が必要な状態
/// 現在、SDKで参照しているサーバー時刻から日時を計算した場合、不正な日時になってしまうため、サーバー時刻を基準とした日時の取得はできない
  NautilusServerTimeStatusNeedUpdate = 1,
/// サーバー時刻は有効な状態
/// サーバー時刻を基準とした日時の取得は可能
  NautilusServerTimeStatusValid = 2,
};
