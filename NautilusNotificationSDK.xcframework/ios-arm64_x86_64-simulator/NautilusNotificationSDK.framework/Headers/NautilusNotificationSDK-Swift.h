...
SWIFT_ENUM_FWD_DECL(NSInteger, NautilusPushType)
@class NautilusRemoteMessage;
@class NSDate;
@class NautilusPushDetailInfo;
SWIFT_ENUM_FWD_DECL(NSInteger, PushOpenSource)
...
/// プッシュ開封記録を実行
/// Objective-Cから呼び出す場合は、こちらのメソッドを利用してください
/// \param pushID プッシュメッセージID
///
/// \param pushIDType プッシュメッセージID種別
///
/// \param openSource 開封元（notification または pushList）
///
/// \param completion 完了ハンドラ
///
- (void)registerOpenPushMessageWithPushID:(NSInteger)pushID pushIDType:(NSInteger)pushIDType openSource:(enum PushOpenSource)openSource completion:(void (^ _Nullable)(BOOL, NSError * _Nullable))completion;
...
/// プッシュ通知の開封元
typedef SWIFT_ENUM(NSInteger, PushOpenSource, open) {
/// OSのシステム通知経由
  PushOpenSourceNotification = 1,
/// アプリ内の通知一覧画面経由
  PushOpenSourcePushList = 2,
};
